...
public interface PessoaMedicoRepository extends R2dbcRepository<PessoaMedico, Long>, PessoaMedicoRepositoryInternal {
    @Query("SELECT * FROM pessoa_medico entity WHERE entity.pessoa_id = :id")
    Flux<PessoaMedico> findByPessoa(Long id);

    @Query("SELECT * FROM pessoa_medico entity WHERE entity.pessoa_id IS NULL")
    Flux<PessoaMedico> findAllWherePessoaIsNull();
