...
public interface PessoaParentescoRepository extends R2dbcRepository<PessoaParentesco, Long>, PessoaParentescoRepositoryInternal {
    @Query("SELECT * FROM pessoa_parentesco entity WHERE entity.pessoa_fisica_id = :id")
    Flux<PessoaParentesco> findByPessoaFisica(Long id);

    @Query("SELECT * FROM pessoa_parentesco entity WHERE entity.pessoa_fisica_id IS NULL")
    Flux<PessoaParentesco> findAllWherePessoaFisicaIsNull();

    @Query("SELECT * FROM pessoa_parentesco entity WHERE entity.pessoa_colaborador_id = :id")
    Flux<PessoaParentesco> findByPessoaColaborador(Long id);

    @Query("SELECT * FROM pessoa_parentesco entity WHERE entity.pessoa_colaborador_id IS NULL")
    Flux<PessoaParentesco> findAllWherePessoaColaboradorIsNull();
