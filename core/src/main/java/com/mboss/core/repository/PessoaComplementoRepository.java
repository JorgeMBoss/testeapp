...
public interface PessoaComplementoRepository extends R2dbcRepository<PessoaComplemento, Long>, PessoaComplementoRepositoryInternal {
    @Query("SELECT * FROM pessoa_complemento entity WHERE entity.id not in (select pessoa_complemento_id from pessoa_fisica)")
    Flux<PessoaComplemento> findAllWherePessoaFisicaIsNull();
