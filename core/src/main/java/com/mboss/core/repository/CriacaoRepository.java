...
public interface CriacaoRepository extends R2dbcRepository<Criacao, Long>, CriacaoRepositoryInternal {
    ...
    @Query("SELECT * FROM criacao entity WHERE entity.empresa_app_id = :id")
    Flux<Criacao> findByEmpresaApp(Long id);

    @Query("SELECT * FROM criacao entity WHERE entity.empresa_app_id IS NULL")
    Flux<Criacao> findAllWhereEmpresaAppIsNull();
