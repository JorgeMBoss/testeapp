...
public class PessoaComplementoResource {
    ...
    public Mono<List<PessoaComplemento>> getAllPessoaComplementos(@RequestParam(required = false) String filter) {
        if ("pessoafisca-is-null".equals(filter)) {
            log.debug("REST request to get all PessoaComplementos where pessoaFisca is null");
            return pessoaComplementoRepository.findAllWherePessoaFiscaIsNull().collectList();
        }
